...
def main_ui():
    ...
    #3차시
        ...
        st.divider()  # 시각적 구분선
            ...
            # ── 역할 선택 & 안내 ───────────────────────────
        roles = {
            "영상 선정 기준":        "우리가 선정한 영상이 왜 분석하기에 적합한지, 어떤 기준으로 선택했는지를 명확하게 전달합니다. 특히, 영상 주제의 특성, 구독자의 수 등을 강조합니다.",
            "회귀분석 결과 및 그래프 설명":   "이차함수 회귀식(a, b, c)의 의미와 함께 그래프의 특징을 설명하여 분석 결과를 이해하기 쉽게 전달합니다. 특히, 꼭짓점, 볼록성, 그래프 형태, y절편, 100만이 되는 시점 등을 강조합니다.",
            "적합도 평가":   "실제 조회수와 예측값의 차이(MSE)를 설명하고 분석 모델이 얼마나 정확한지를 명료하게 전달합니다. 특히, 수치가 낮을수록 예측 정확도가 높다는 점 등 오차의 의미를 이해하기 쉽게 전달합니다.",
            "마케팅 전략":   "분석 결과를 토대로 효과적인 마케팅 전략을 제안하고 설득력 있게 설명합니다. 특히, 광고비를 어떻게 쓸지, 영상 업로드 최적 타이밍 등을 강조하여 설득력 확보합니다.",
            "느낀점 및 종합 정리":        "분석 과정에서 배운 점과 한계점, 앞으로의 개선 방향 등을 종합적으로 제시하여 발표를 마무리합니다. 특히, 개인적 및 조 전체가 경험한 성장과 배움을 강조, 현실적이고 구체적인 개선 방안 제시합니다.",
        }
        my_role = st.selectbox("역할을 선택하세요", list(roles.keys()), key="role_select")
        st.info(f"**내 역할 가이드:**  {roles[my_role]}")

        script_templates = {
            "영상 선정 기준":      "예시) 안녕하세요 영상 선정 기준을 발표할 ○○○입니다. 저희 조는 영상 주제가 학생들이 좋아할 만한 재미있는 콘텐츠인지를 가장 중요하게 생각했습니다. 또한, 채널의 구독자 수가 너무 많지 않은 중소형 채널의 영상을 골라 더 의미 있게 분석하려 했습니다. 결과적으로 선택한 영상은 유명한 영화 장면을 재밌게 패러디한 콘텐츠로 저희 또래가 관심을 가질 만해서 선정하게 되었습니다..…",
            "회귀분석 결과 및 그래프 설명": "예시) 안녕하세요, 회귀분석 결과를 설명할 ○○○입니다. 저희 조가 구한 이차함수 회귀식은 y=~ 로, x² 앞의 계수가 양수여서 그래프는 아래로 볼록한 형태가 되었습니다. 그래프의 꼭짓점은 아래로 내려온 형태가 아니라 시간이 지날수록 계속 상승하는 모양을 보였고 초기 조회수를 나타내는 y절편 값은 15만 회로 나타났습니다. 또한 그래프가 위로 볼록한 형태여서 100만 회를 넘는 시점이 명확히 나타나진 않았지만 계산을 통해 약 40시간 이후에 100만 회를 달성할 것으로 예측할 수 있었습니다. …",
            "적합도 평가": "예시) 안녕하세요, 적합도 평가 결과를 설명할 ○○○입니다. 저희 조는 모델의 정확성을 확인하기 위해 MSE만 사용하여 평가를 진행했습니다. MSE는 예측값과 실제값의 평균적인 차이를 나타내는데, 저희 모델의 MSE 값은 약 52만으로 나타났습니다. 이 수치는 분석 모델이 실제 조회수와 큰 차이 없이 잘 예측하고 있음을 나타내며 저희가 만든 모델이 꽤 정확하다고 판단할 수 있었습니다.…",
            "마케팅 전략": "예시) 안녕하세요, 마케팅 전략을 설명할 ○○○입니다. 저희는 초반 조회수 상승을 효과적으로 활용하기 위해 구글 Ads(구글 광고)를 초반 10시간에 집중적으로 집행하는 전략을 세웠습니다. 구글 Ads는 시청자들의 나이, 관심사, 접속 시간 등을 바탕으로 타겟팅할 수 있어서 초반의 조회수를 효과적으로 높이는 데 큰 도움이 될 것으로 기대됩니다. 영상 업로드 시간은 학생들이 주로 많이 접속하는 주말 오후 5시에서 6시 사이를 추천합니다.…",
            "느낀점 및 종합 정리":      "예시) 안녕하세요, 느낀점을 발표할 ○○○입니다. 저는 원래 수학이 어렵기만 하고 실생활과는 관련이 없다고 생각했는데 우리가 매일 보는 유튜브 영상과도 연결될 수 있다는 점이 놀라웠습니다. 특히 직접 데이터를 분석하고 마케팅 전략까지 세워보니 수학이 실제로 우리 생활에 정말 중요하다는 걸 느꼈습니다. 다음에 다시 한다면 더 다양한 영상을 분석해보고 싶습니다. 감사합니다!…"
        }
